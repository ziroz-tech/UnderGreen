...
- `schedule_rumors.csv`: 月間スケジュールの噂。日付、対象市場、対象需要軸、対象作物、発生確率、需要指標の押し上げ量を管理します。
...
- `property_comments.csv`: 不動産カードに表示されるキドの物件コメント。`tier` ごとに複数行を設定でき、セール物件用の追記も編集できます。
...
- `ui_text.csv`: 画面固定文言。`selector` がある行は該当HTMLを上書きし、`selector` が空の行はJS内メッセージ用キーです。調達端末の固定文言やブローカー紹介文もここで編集します。
...
- 予定表の噂は `schedule_rumors.csv` で編集します。`rumor` は噂の概要説明、`comment` は「〇〇が高く売れるかもしれない。」のような狙い作物の明示に使います。`signalBoost` は噂の有効日に、その市場需要指標を最低どこまで押し上げるかです。`priceBoost` は噂対象作物の当日価格倍率です。
